...
from tools.dataloaderx import DataLoaderX
...
from tools.to_gpu import to_gpu
from utils.load_weight import load_segnet_weight
...
def parse_args():
    ...
    parser.add_argument('--gpu_no', default=6, type=int)
    ...
    #training setting
    parser.add_argument('--cfg', default='./Config/vitb_cityscale.py', type=str)
    parser.add_argument('--save_path', default='work_dir/ckpt_seg_vib_cityscale', type=str)
    parser.add_argument('--load_path', default='work_dir/ckpt_seg_vib_cityscale', type=str)
    parser.add_argument('--load_no', nargs='+', type=str, help='the no of the pretrain weight')
    parser.add_argument('--train_vector', default='0', type=int)
...
def get_model(net, is_gpu=False, is_multigpu=False):
    ...
    if is_multigpu:
        ...
        unused = (not getattr(cfg, 'train_vector', False)) and (getattr(cfg, 'backbone', '') == 'sam2_hiera_b+')
        net = DDP(net, device_ids=[GPUNO], find_unused_parameters=unused)
    return net
...
def train():
    ...
    def warm_up_cosine_lr(iteration):
        ...
        min_lr_ratio = 0.2  # 最小学习率比例为初始学习率的1/10
        if cfg.is_debug:
            min_lr_ratio = 1
        if iteration <= warm_up:
            return iteration / warm_up
        else:
            progress = (iteration - warm_up) / (max_iter - warm_up)
            cosine_decay = 0.5 * (math.cos(progress * math.pi) + 1)
            return min_lr_ratio + (1 - min_lr_ratio) * cosine_decay

    lr_sche = torch.optim.lr_scheduler.LambdaLR(optimizer, warm_up_cosine_lr)
    loss_fun = build_loss(cfg)
    if GPU or MULTIGPU:
    ...
    for epoch_no in range(cfg.epoch_num):
        ...
        if ISMAINPROCESS and (epoch_no+1)%1==0:
            net_weight = net.module.state_dict() if cfg.is_multigpu else net.state_dict()
            save_weight = os.path.join(cfg.save_path, 'para_{}.pth'.format(str(epoch_no+1)))
            torch.save(net_weight, save_weight)
    if ISMAINPROCESS:
        net_weight = net.module.state_dict() if cfg.is_multigpu else net.state_dict()
        save_weight = os.path.join(cfg.save_path, 'para_{}.pth'.format(str(cfg.epoch_num)))
        torch.save(net_weight, save_weight)
    if MULTIGPU:
