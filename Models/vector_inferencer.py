...
class VectorInferencer(nn.Module):
    ...
    def combine_segmap(self, patch_segmaps):
        '''
    ...
    def patch_sample_points(self, points):
        ...
        for p in range(self.patch_num):
            ...
            patch_points = points[patch_mask] - self.patch_start_grid[p][None, :]
    ...
    def patch_sample_points_single(self, points):
        patch_centers = self.patch_start_grid + torch.tensor([self.patch_w / 2, self.patch_h / 2],
                                                             device=self.patch_start_grid.device)
    ...
    def combine_sample_points(self, patch_points, patch_point_inds):
        patch_start_grid = self.patch_start_grid.unsqueeze(1)
