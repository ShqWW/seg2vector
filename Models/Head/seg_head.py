...
class SegHead(nn.Module):
    ...
    def forward_testing(self, feats):
        ...
        for i in range(num_splits):
            ...
            # mask_logits_split = mask_logits_split[:, 1:2, ...]  # only keep the first channel for binary segmentation
            # import cv2
            # img_save = mask_logits_split[0, 0, ...].detach().cpu().numpy()
            # cv2.imwrite(f"mask_logits_split.png", img_save * 255)
            # exit(0)
