...
def TOPOGenerateStartingPoints(OSMMap, check=True, density=0.00050, region=None, image=None, direction=False,
                               metaData=None, mergin=0.07):
    result = []
                    ...
                    if alpha * dist >= dists[j] and alpha * dist <= dists[j + 1]:
                        ...
                        if img != None:
                            ...
                            if x > 0 and x < sizex and y > 0 and y < sizey:
                                if img[x, y] > 0:
                                    result.append((lat, lon, node_list[j], node_list[j + 1], alpha * dist - dists[j],
                            ...
                            if lat - region[0] > lat_mergin and region[2] - lat > lat_mergin and lon - region[
                                1] > lon_mergin and region[3] - lon > lon_mergin:
                                result.append((lat, lon, node_list[j], node_list[j + 1], alpha * dist - dists[j],
                                               dists[j + 1] - alpha * dist))

    for _, edge in OSMMap.edges.items():
        svgEdges.append(
            (OSMMap.nodes[edge[0]][0], OSMMap.nodes[edge[0]][1], OSMMap.nodes[edge[1]][0], OSMMap.nodes[edge[1]][1]))
...
def TOPOGeneratePairs(GPSMap, OSMMap, OSMList, threshold=0.00010, region=None, single=False, edgeids=None):
    ...
    for i in range(len(OSMList)):
        ...
        possible_edges = list(
            idx.intersection((lat - threshold * 2, lon - threshold * 2, lat + threshold * 2, lon + threshold * 2)))
        ...
        if min_edge != -1:
            ...
            result[i] = [edgeid, n1, n2, distance((lat1, lon1), (lat, lon)), distance((lat2, lon2), (lat, lon)), lat,
                         lon]
...
def TOPOGenerateList(GPSMap, OSMMap, check=True, threshold=0.00010, region=None, image=None, direction=False):
    ...
    for idthis in OSMMap.nodes.keys():
        x, y = Coord2Pixels(OSMMap.nodes[idthis][0], OSMMap.nodes[idthis][1], region[0], region[1], region[2],
                            region[3], sizex, sizey)
        if x > 0 and x < sizex and y > 0 and y < sizey:
            if img[x, y] > 0:
                idx.insert(idthis,
                           (OSMMap.nodes[idthis][0], OSMMap.nodes[idthis][1], OSMMap.nodes[idthis][0] + 0.000001,
...
def TOPO(GPSMap, OSMMap, step=0.00005, r=0.00300, num=1000, threshold=0.00020, region=None):
    idx = index.Index()
    for idthis in OSMMap.nodes.keys():
        idx.insert(idthis, (OSMMap.nodes[idthis][0], OSMMap.nodes[idthis][1], OSMMap.nodes[idthis][0] + 0.000001,
                            OSMMap.nodes[idthis][1] + 0.000001))
    ...
    for i in range(num):
        while True:
            ...
            possible_nodes = list(
                idx.intersection((lat - threshold, lon - threshold, lat + threshold, lon + threshold)))
...
def TOPO121(topo_result, roadgraph):
    ...
    for ind in range(len(topo_result)):
        ...
        for can_id in candidate:
            t_gpsn1, t_gpsn2, t_gpsd1, t_gpsd2 = topo_result[can_id][4], topo_result[can_id][5], topo_result[can_id][6], \
            topo_result[can_id][7]

            d = roadgraph.distanceBetweenTwoLocation((gpsn1, gpsn2, gpsd1, gpsd2), (t_gpsn1, t_gpsn2, t_gpsd1, t_gpsd2),
...
def TOPOWithPairs(GPSMap, OSMMap, GPSList, OSMList, step=0.00005, r=0.00300, threshold=0.00015, region=None,
                  outputfile="tmp.txt", one2oneMatching=True, metaData=None):
    i = 0
    precesion_sum = 0
    recall_sum = 0
    ...
    for k, itemGPS in GPSList.items():
        ...
        ts1 = time()
        marbles = GPSMap.TOPOWalk(1, step=step, r=r, direction=False, newstyle=True, nid1=gpsn1, nid2=gpsn2,
                                  dist1=gpsd1, dist2=gpsd2)

        # for recall
        holes = OSMMap.TOPOWalk(1, step=step, r=r, direction=False, newstyle=True, nid1=osmn1, nid2=osmn2, dist1=osmd1,
                                dist2=osmd2, metaData=metaData)  # remove holes in tunnel

        # for precision
        holes_bidirection = OSMMap.TOPOWalk(1, step=step, r=r, direction=False, newstyle=True, nid1=osmn1, nid2=osmn2,
                                            dist1=osmd1, dist2=osmd2, bidirection=True,
        ...
        for j in range(len(marbles)):
            idx_marbles.insert(j, (marbles[j][0] - 0.00001, marbles[j][1] - 0.00001, marbles[j][0] + 0.00001,
                                   marbles[j][1] + 0.00001))

        for j in range(len(holes)):
            idx_holes.insert(j, (holes[j][0] - 0.00001, holes[j][1] - 0.00001, holes[j][0] + 0.00001,
                                 holes[j][1] + 0.00001))

        for j in range(len(holes_bidirection)):
            idx_holes_bidirection.insert(j, (holes_bidirection[j][0] - 0.00001, holes_bidirection[j][1] - 0.00001,
                                             holes_bidirection[j][0] + 0.00001, holes_bidirection[j][1] + 0.00001))
        ...
        for marble in marbles:
            rr = threshold * 1.8
            possible_holes = list(
                idx_holes_bidirection.intersection((marble[0] - rr, marble[1] - rr, marble[0] + rr, marble[1] + rr)))
        ...
        with open(outputfile, "a") as fout:
            fout.write(
                str(i) + " " + str(lat) + " " + str(lon) + " " + str(gpsn1) + " " + str(gpsn2) + " Precesion " + str(
                    precesion) + " Recall " + str(recall) + " Avg Precesion " + str(
                    precesion_sum / (i + 1)) + " Avg Recall " + str(recall_sum / (i + 1)) + " \n")
    ...
    try:
        with open(outputfile, "a") as fout:
            fout.write(str(p) + " " + str(r) + " " + str(len(new_topoResult) / float(len(OSMList))) + " " + str(
                r * len(new_topoResult) / float(len(OSMList))) + "\n")
...
def TOPOWithPairsNew(GPSMap, OSMMap, GPSList, OSMList, step=0.00005, r=0.00300, threshold=0.00015, region=None,
                     outputfile="tmp.txt", one2oneMatching=True, base_n=None, svgname="", soft=True, CheckGPS=None):
    i = 0
    precesion_sum = 0
    recall_sum = 0
    ...
    for k, itemGPS in GPSList.items():
        ...
        # showTOPO.RenderSVG(marbles, holes_bidirection, matched_marbles, matched_holes, lat, lon, 0.00500,
        ...
        if len(marbles) == 0:
            ...
            smooth_precision = 1.0 - (cost + (len(marbles) - matchedNum) * threshold * 1.15) / (
...
def TOPOWithList(GPSMap, OSMMap, pairs, step=0.00005, r=0.00300, threshold=0.00015, region=None, outputfile="tmp.txt"):
    ...
    for min_node in pairs.keys():
        ...
        # showTOPO.RenderSVG(marbles, holes, lat, lon, 0.00500,
        ...
        with open(outputfile, "a") as fout:
            fout.write(str(i) + " MapNodeID " + str(nid) + " OSMNodeID " + str(min_node) + " Precesion " + str(
                precesion) + " Recall " + str(recall) + " Avg Precesion " + str(
                precesion_sum / (i + 1)) + " Avg Recall " + str(recall_sum / (i + 1)) + " \n")
